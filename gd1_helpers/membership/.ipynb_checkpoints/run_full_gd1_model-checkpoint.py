...
def __name__=='__main__':
    ...
    if sep == 'xsmall':
        knot_sep = 5
        dens_step = np.array([2, 0.2]) #small, goes with knot_step=5
    elif sep == 'small':
        knot_sep = 10
        dens_step = np.array([4, 0.4]) #medium, goes with knot_step=10
    elif sep == 'medium':
        knot_sep = 15
        dens_step=np.array([6, 0.6]) #large, goes with knot_step=15
    elif sep == 'large':
        knot_sep = 20
        dens_step = np.array([8, 0.8])
    ...
    stream_opt_pars, _ = StreamDensModel.optimize(
    ...
    # mix_opt_pars, _ = StreamMixtureModel.optimize(
    ...
    with open('../data/full_model_opt_params_large_sep.pkl', 'wb') as output_file:
